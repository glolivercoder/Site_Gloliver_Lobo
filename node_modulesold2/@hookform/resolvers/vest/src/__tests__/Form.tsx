...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="9p_1iso">
      <input {...register("username")} data-oid="dep7a71" />
      {errors.username && (
        <span role="alert" data-oid="-.:j.68">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="ekj0cvn" />
      {errors.password && (
        <span role="alert" data-oid="gnpi:_b">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="g_i9o:n">
        submit
      </button>
    </form>
  );
...
test("form's validation with Vest and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="ya59-7m" />);
