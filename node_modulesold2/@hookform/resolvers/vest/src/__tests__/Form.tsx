...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="to::j5:">
      <input {...register("username")} data-oid="arpyccb" />
      {errors.username && (
        <span role="alert" data-oid="4xfm34_">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="utp7hir" />
      {errors.password && (
        <span role="alert" data-oid="2qbvfc3">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="89vkzjp">
        submit
      </button>
    </form>
  );
...
test("form's validation with Vest and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="687xlz8" />);
