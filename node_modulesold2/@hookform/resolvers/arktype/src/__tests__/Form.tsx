...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="brnt95:">
      <input {...register("username")} data-oid="j4669vs" />
      {errors.username && (
        <span role="alert" data-oid="27b:_2-">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="veztqbj" />
      {errors.password && (
        <span role="alert" data-oid="-i:os-v">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="058vpcv">
        submit
      </button>
    </form>
  );
...
test("form's validation with arkType and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="nfvsx82" />);
