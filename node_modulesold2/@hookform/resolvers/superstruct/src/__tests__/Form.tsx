...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="hqi2:ns">
      <input {...register("username")} data-oid="m_t15:r" />
      {errors.username && (
        <span role="alert" data-oid="9zq33te">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="klq-vyo" />
      {errors.password && (
        <span role="alert" data-oid="6g1ypve">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="1op3yjn">
        submit
      </button>
    </form>
  );
...
test("form's validation with Superstruct and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="64xcvou" />);
