...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="92erv47">
      <input {...register("username")} data-oid="v8j:yjs" />
      {errors.username && (
        <span role="alert" data-oid="ldx5suq">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="23owtix" />
      {errors.password && (
        <span role="alert" data-oid="5rh-fr:">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="ulgfy_s">
        submit
      </button>
    </form>
  );
...
test("form's validation with Typebox and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="_cjf.e9" />);
