...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="k_mf6.s">
      <input {...register("username")} data-oid="xl.oa4s" />
      {errors.username && (
        <span role="alert" data-oid="cvfhg9:">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="7u6etla" />
      {errors.password && (
        <span role="alert" data-oid="jm-trnd">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="kd3of9l">
        submit
      </button>
    </form>
  );
...
test("form's validation with Yup and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="tm7bb2d" />);
