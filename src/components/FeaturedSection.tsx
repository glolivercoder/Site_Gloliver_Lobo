...
const MediaPlayer = ({ url, type, waveformStyle = "bars" }: { url: string; type: string; waveformStyle?: "bars" | "wave" | "mirror" }) => {
  // YouTube
  ...
  if (type === 'audio' || url.match(/\.(mp3|wav|ogg)$/i)) {
    return (
      <div className="p-4">
        <AudioVisualizer url={url} autoPlay={true} waveformStyle={waveformStyle} />
      </div>
    );
  }
...
export const FeaturedSection = () => {
  ...
  const [waveformStyle, setWaveformStyle] = useState<"bars" | "wave" | "mirror">("bars");
    ...
    const loadFeatured = () => {
      ...
      if (stored) {
        try {
          ...
          setAllPages(parsed.map((page: any[]) => 
            page.map((item: any, index: number) => ({
              ...item,
              image: item.type === 'image' && item.url ? item.url : defaultFeatured[index % 8]?.image || defaultFeatured[0].image
            }))
          ));
        } catch (e) {
    ...
    <>
      ...
      <Dialog open={!!selectedMedia} onOpenChange={() => setSelectedMedia(null)}>
        <DialogContent className="max-w-4xl bg-deep-black/95 border-golden/20">
          <DialogHeader>
            <DialogTitle className="text-golden text-2xl">{selectedMedia?.title}</DialogTitle>
          ...
          <div className="aspect-video w-full bg-black rounded-lg overflow-hidden">
            {selectedMedia?.url && (
              <MediaPlayer url={selectedMedia.url} type={selectedMedia.type} waveformStyle={waveformStyle} />
            )}
          </div>
        </DialogContent>
      </Dialog>
