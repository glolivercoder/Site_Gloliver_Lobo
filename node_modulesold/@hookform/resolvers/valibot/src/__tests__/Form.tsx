...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="micpx3_">
      <input {...register("username")} data-oid="yju28yu" />
      {errors.username && (
        <span role="alert" data-oid="d:_qfmz">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="l409ehu" />
      {errors.password && (
        <span role="alert" data-oid="7ul88t:">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="1b95-ub">
        submit
      </button>
    </form>
  );
...
test("form's validation with Valibot and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="vbg:3h1" />);
