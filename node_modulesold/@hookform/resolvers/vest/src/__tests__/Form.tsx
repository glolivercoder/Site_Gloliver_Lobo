...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="hvqjo8y">
      <input {...register("username")} data-oid="okrqqkl" />
      {errors.username && (
        <span role="alert" data-oid="av07x_d">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="2a-_td." />
      {errors.password && (
        <span role="alert" data-oid="tp3o0.3">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="9bpbnjt">
        submit
      </button>
    </form>
  );
...
test("form's validation with Vest and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="s72acno" />);
