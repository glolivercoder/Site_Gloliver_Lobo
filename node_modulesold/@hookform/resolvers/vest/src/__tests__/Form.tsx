...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="iprqogk">
      <input {...register("username")} data-oid="-afoaug" />
      {errors.username && (
        <span role="alert" data-oid="hpoxe00">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="lpt7y5u" />
      {errors.password && (
        <span role="alert" data-oid="606411n">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="3xt3q6b">
        submit
      </button>
    </form>
  );
...
test("form's validation with Vest and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="x09x8iw" />);
