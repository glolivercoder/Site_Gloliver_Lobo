...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="ded0xq2">
      <input {...register("username")} data-oid="vwiitrb" />
      {errors.username && (
        <span role="alert" data-oid=".rainr-">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="5reqmku" />
      {errors.password && (
        <span role="alert" data-oid="mswm3c:">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="x7ccif:">
        submit
      </button>
    </form>
  );
...
test("form's validation with TypeSchema and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="u3iwhg2" />);
