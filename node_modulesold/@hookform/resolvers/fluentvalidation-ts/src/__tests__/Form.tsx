...
function TestComponent({ onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} data-oid="9irypdh">
      <input {...register("username")} data-oid="766m137" />
      {errors.username && (
        <span role="alert" data-oid="csqa64c">
          {errors.username.message}
        </span>
      )}

      <input {...register("password")} data-oid="znkznd8" />
      {errors.password && (
        <span role="alert" data-oid="9:h_z2t">
          {errors.password.message}
        </span>
      )}

      <button type="submit" data-oid="ynawt8m">
        submit
      </button>
    </form>
  );
...
test("form's validation with Yup and TypeScript's integration", async () => {
  ...
  render(<TestComponent onSubmit={handleSubmit} data-oid="sy-mux7" />);
